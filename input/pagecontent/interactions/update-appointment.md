...
- the timing (```start```, ```end```, ```minutesDuration```, ```slot```) — use the [Postpone Appointment](functional-transactions.html#postpone-appointment) interaction instead;
- the service (```serviceType``` and the referenced HealthcareService) — cancel the Appointment (see [Cancel Appointment](functional-transactions.html#cancel-appointment)) and book a new one (see [Book Appointment](functional-transactions.html#book-appointment-book)) instead.
